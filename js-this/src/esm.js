...
export default [
  ...
  },
  {
    name: 'export default',
    open: false,
    reg: /(§{2}keywordExport\d+§{2})\s*(§{2}keywordDefault\d+§{2})\s*([^\s]*)+\s*(§{2}(endLine|endExpression|endPonctuation)\d+§{2})/,
    id: (value, matches, typedExpressions, expressions) => {
  ...
  },
  {
    name: 'export vars',
    open: false,
    reg: /(§{2}keywordExport\d+§{2})\s*(§{2}(keywordConst|keywordLet)\d+§{2})\s*([^§])+\s*(§{2}operatorsetter\d+§{2})/,
    id: (value, matches, typedExpressions, expressions) => {
      const id = `§§export${gen.next().value}§§`
  ...
  },
  {
    name: 'aliased import',
    open: false,
    reg: /\s*(§{2}keywordImport\d+§{2})\s*(§{2}allAs\d+§{2})\s+([^§]*)+\s+(§{2}keywordFrom\d+§{2})\s*(§{2}string\d+§{2})\s*(§{2}(endLine|endExpression|endPonctuation)\d+§{2})?/,
    id: (value, matches, typedExpressions, expressions) => {
  ...
  },
  {
    name: 'ambient import',
    open: false,
    reg: /\s*(§{2}keywordImport\d+§{2})\s+(§{2}string\d+§{2})\s*(§{2}(endLine|endExpression|endPonctuation)\d+§{2})?/,
    id: (value, matches, typedExpressions, expressions) => {
  ...
  },
  {
    ...
    open: false,
    reg: /\s*(§{2}keywordImport\d+§{2})\s+([^§]*)+\s*(§{2}(endLine|endExpression|endPonctuation)\d+§{2})/,
    id: (value, matches, typedExpressions, expressions) => {
      const id = `§§import${gen.next().value}§§`
  ...
  },
  {
    ...
    open: false,
    reg: /\s*(§{2}keywordExport\d+§{2})\s+([\w\d]*)+\s+(§{2}keywordFrom\d+§{2})\s*(§{2}string\d+§{2})\s*(§{2}(endLine|endExpression|endPonctuation)\d+§{2})?/,
    id: (value, matches, typedExpressions, expressions) => {
      const id = `§§import${gen.next().value}§§`
  ...
  },
  {
    name: 'default import',
    open: false,
    reg: /\s*(§{2}keywordImport\d+§{2})\s+([\w\d]*)+\s+(§{2}keywordFrom\d+§{2})\s*(§{2}string\d+§{2})\s*(§{2}(endLine|endExpression|endPonctuation)\d+§{2})?/,
    id: (value, matches, typedExpressions, expressions) => {
      const id = `§§import${gen.next().value}§§`
      const [input, imp, key, f, id2] = matches;
      expressions[id] = value;
...

  {
    ...
    open: false,
    reg: /(§{2}keywordImport\d+§{2})\s*(§{2}importCandidate\d*§{2})\s*(§{2}allAs\d+§{2})\s*([^§]*)+\s*(§{2}keywordFrom\d+§{2})\s*(§{2}string\d+§{2})\s*(§{2}(endLine|endExpression|endPonctuation)\d+§{2})?/,
    id: (value, matches, typedExpressions, expressions) => {
      const id = `§§import${gen.next().value}§§`
  ...
  },
  {
    name: 'default and aliased import',
    open: false,
    reg: /(§{2}keywordImport\d+§{2})\s*(§{2}allAs\d*§{2})\s*(§{2}importCandidate\d+§{2})\s*([^§]*)+\s*(§{2}keywordFrom\d+§{2})\s*(§{2}string\d+§{2})\s*(§{2}(endLine|endExpression|endPonctuation)\d+§{2})?/,
    id: (value, matches, typedExpressions, expressions) => {
      const id = `§§import${gen.next().value}§§`
  ...
  },
  {
    name: 'object import',
    open: false,
    reg: /\s*(§{2}keywordImport\d+§{2})\s*(§{2}blockImport\d+§{2})\s*(§{2}keywordFrom\d+§{2})\s*(§{2}string\d+§{2})\s*(§{2}(endLine|endExpression|endPonctuation)\d+§{2})?/,
    id: (value, matches, typedExpressions, expressions) => {
      const id = `§§import${gen.next().value}§§`
      const [input, imp, key, f, id2] = matches;
      expressions[id] = value;
  ...
  },
  {
    ...
    open: false,
    reg: /\s*(§{2}keywordImport\d+§{2})\s*(§{2}blockImport\d+§{2})\s*([^§]*)+\s*(§{2}keywordFrom\d+§{2})\s*(§{2}string\d+§{2})\s*(§{2}(endLine|endExpression|endPonctuation)\d+§{2})?/,
    id: (value, matches, typedExpressions, expressions) => {
      const id = `§§import${gen.next().value}§§`
  ...
  },
  {
    name: 'default and object import',
    open: false,
    reg: /\s*(§{2}keywordImport\d+§{2})\s*(§{2}importCandidate\d+§{2})\s*(§{2}blockImport\d+§{2})\s*(§{2}keywordFrom\d+§{2})\s*(§{2}string\d+§{2})\s*(§{2}(endLine|endExpression|endPonctuation)\d+§{2})?/,
    id: (value, matches, typedExpressions, expressions) => {
      const id = `§§import${gen.next().value}§§`
  ...
  },
  {
    ...
    id: (value, matches, typedExpressions, expressions) => {
      const id = ``
      const UnsupportedSyntaxOfImport = new SyntaxError(`[Ogone] this syntax of import is not supported\n`)
