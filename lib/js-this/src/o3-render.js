...
export default [
  ...
  // TODO
  {
    name: "reflection",
    open: false,
    reg: /(§{2}keywordThis\d+§{2})\s*((§{2}(identifier|array)\d+§{2})+)\s*(§{2}arrowFunction\d+§{2})\s*(§{2}block\d+§{2})/,
  ...
  },
  {
    name: "reflection",
    open: false,
    reg: /(§{2}keywordThis\d+§{2})\s*((§{2}(identifier|array)\d+§{2})+)\s*(§{2}arrowFunction\d+§{2})\s*([^\s]+)+\s*(§{2}(endLine|endExpression|endPonctuation)\d+§{2})/,
    id: (value, matches, typedExpressions, expressions) => {
      const id = `§§reflection${gen.next().value}§§`;
      const [input, keywordThis, identifier] = matches;
      const fnbody = matches.find(
  ...
  },
  {
    name: "reflection",
    open: false,
    reg: /(§{2}keywordThis\d+§{2})\s*(§{2}identifier\d+§{2})\s*(§{2}arrowFunction\d+§{2})\s*([^\s]+)+/,
    id: (value, matches, typedExpressions, expressions) => {
...
not supported in this version of Ogone
  ...
  // use relative path
  {
    name: "declarations",
    open: false,
    reg: /(§{2}keywordUse\d+§{2})\s*((§{2}ponctuation)([^\s]*)+)\s*(§{2}keywordAs\d+§{2})\s*(§{2}string\d+§{2})(\s*§{2}endPonctuation\d+§{2})*/,
    id: (value, matches, typedExpressions, expressions) => {
  ...
  // use remotes components
  {
    name: "declarations",
    open: false,
    reg: /(§{2}keywordUse\d+§{2})\s+((https|http)(§{2}optionDiviser\d+§{2}\/{2})([^\s]*)+)\s+(§{2}keywordAs\d+§{2})\s*(§{2}string\d+§{2})(\s*§{2}endPonctuation\d+§{2})*/,
    id: (value, matches, typedExpressions, expressions) => {
      const id = `§§use${gen.next().value}§§`;
      let path = templateReplacer(matches[2], expressions);
      typedExpressions.use[id] = {
